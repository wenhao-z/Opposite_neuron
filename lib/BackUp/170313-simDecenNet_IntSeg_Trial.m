function [InputSet, NetStat] = simDecenNet_IntSeg_Trial(InputSet, NetPars, outArgs)
...
% Author: Wen-Hao Zhang, Dec-30-2016
...
%     O(:,1:2,1) = O(:,1:2,1) + 30;
    ...
    for t = 1: size(Iext, 3) - 1
        ...
        theta = 4;
        Urec(Urec<theta) = 0;
        Urec(Urec>=theta) = Urec(Urec>=theta) - theta;
        ...
        Otmp = bsxfun(@rdivide, Urec, 1+ NetPars.k *divU);
        %         Otmp(Otmp<8) = 0;
        O(:,:,t+1) = Otmp;
    end
