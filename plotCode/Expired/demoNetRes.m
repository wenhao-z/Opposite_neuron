...
NetPars.NoiseFree = 1; % 1: no noise; 0: add noise;
% NetPars.Posi = [120, 60]';
NetPars.Posi = [0, -60]';
...
for iterPar = 1: numel(parGrid)
    ...
    if netpars.Jrc*(1 + netpars.JrpRatio) > netpars.Jc
        ...
        % Bump position
        O = InputSet.O(:,:, 10*NetPars.tau/ NetPars.dt+1 : end);
...
for iterCond = 1:3
    ...
    for iterGroup = IdxGroup
        ...
        % Catesian coordinate
        plot(hAxe(iterCond+3), NetPars.PrefStim, RateAvg{iterCond}(:, iterGroup), ...
            'color', cSpec(iterGroup, :));
        plot(hAxe(iterCond+3), meanNetSim(iterCond,1, iterGroup) * ones(1,2), ...
            [0, max(RateAvg{iterCond}(:, iterGroup))], '--k')
    end
...
polar(NetPars.Posi(1)* pi/ 180 * ones(1,2), ...
    [0, max(RateAvg{1}(:, iterGroup))], '--k')
polar(NetPars.Posi(2)* pi/ 180 * ones(1,2), ...
    [0, max(RateAvg{1}(:, iterGroup))], '--k')

% axes(hAxe(4))
% plot(NetPars.Posi(1) * ones(1,2), [0, rPlot(end)])

set(hAxe(4:6), 'xlim', 180*[-1 1], 'xtick', -180:90:180, ...
    'ylim', [0, rPlot(end)])